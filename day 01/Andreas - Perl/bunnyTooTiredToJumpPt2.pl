...
sub TrackPath { #($startX, $startY, $stopX, $stopY)
	(my $startX, my $startY, my $stopX, my $stopY) = @_;
	
	if ($startX == $stopX){
		#move in Y direction
		(my $from, my $to) = (sort { $a <=> $b } ($startY, $stopY));

		my $i = $from+1;
		$visitedLocations{$startX} = {} if ! exists $visitedLocations{$startX};
		while ($i <= $to and ! exists $visitedLocations{$startX}{$i}){
			$visitedLocations{$startX}{$i} = 1;
		...
		if ($i < $to){
			print "found ($startX, $i) - " . (abs($startX) + abs($i)) . "\n";
			exit;
		}

	}elsif ($startY == $stopY){
		#move in X direction
		(my $from, my $to) = (sort { $a <=> $b } ($startX, $stopX));

		my $i = $from+1;
		while ($i <= $to and ! exists $visitedLocations{$i}{$startY}){
			$visitedLocations{$i} = {} if ! exists $visitedLocations{$i};
			$visitedLocations{$i}{$startY} = 1;
		...
		if ($i < $to){
			print "found ($i, $startY) - Dist:" . (abs($i) + abs($startY)) . "\n";
			exit;
		}

	}else{
		print "you can only move in straight lines, thus ($startX,$startY) -> ($stopX,$stopY) is not possible.\n";
		exit;
	}
}
...
sub TraversePath { #($input, $startX, $startY, $startDirection)
	...
	foreach (@inputs){
		...
		TrackPath($currX,$currY,$newX,$newY);
		#print"\n";
